...
void RecherchePar_Nom(repertoire_de_patients *rep) {
    ...
    scanf("%s", nom);
    for (int i = 0; i < rep->nbPatients; i++) {
        if (strcmp(nom, rep->liste[i].nom) == 0) {
            printf("\nPatient %s Trouve ! \n", nom);
            tv++;
            break;
        }
...
void Afficher_le_plus_jeune_st_le_plus_age(repertoire_de_patients *rep) {
    if (rep->nbPatients == 0) {
        printf("Aucun patient dans le repertoire.\n");
        return;
    }
...
void Carte_Patient(repertoire_de_patients *rep) {
    char nom[50];
...
void afficherParGroupeSanguin(repertoire_de_patients *rep) {
    char groupe[10];
    int trouve = 0;
    printf("Saisir le groupe sanguin a rechercher : ");
    scanf("%s", groupe);

    printf("| %-15s | %-15s | %-10s |\n", "Nom", "Prenom", "Groupe");
    for (int i = 0; i < rep->nbPatients; i++) {
        if (strcmp(rep->liste[i].groupe_sanguin, groupe) == 0) {
            printf("%-15s | %-15s | %-10s \n", rep->liste[i].nom, rep->liste[i].prenom, rep->liste[i].groupe_sanguin);
            trouve++;
        }
    }
    if (trouve == 0) printf("Aucun patient trouve avec le groupe %s.\n", groupe);
}

void statistiquesPatients(repertoire_de_patients *rep) {
    printf("\n--- Statistiques du Cabinet ---\n");
    printf("Nombre total de patients enregistres : %d\n", rep->nbPatients);
...
int main() {
    ...
    do {
        ...
        printf("10. Charger\n");
        printf("11. Carte Patient \n");
        printf("12. Afficher par groupe sanguin\n");
        ...
        switch (choix) {
            ...
            case 11: Carte_Patient(&rep); break;
            case 12: afficherParGroupeSanguin(&rep); break;
            case 13: statistiquesPatients(&rep); break;
            case 14: printf("Au revoir !\n"); break;
